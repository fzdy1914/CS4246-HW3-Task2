    ...
    for episode in range(max_episodes):
        ...
        if episode % print_interval == 0 and episode > 0:
            print("[Curriculum {} Type {} Episode {}]\tavg rewards : {:.3f},\tavg loss: : {:.6f},\tbuffer size : {},\tepsilon : {:.1f}%".format(
                            curriculum_num, train_type, episode, np.mean(rewards[print_interval:]), np.mean(losses[print_interval * 10:]), len(memory), epsilon * 100))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', dest='train', action='store_true', help='generate a basic agent')
